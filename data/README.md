# Data sources
This directory contains yaml files of data sources to ingest into the Rubin_RAG Weaviate vector database

There is one file per data sources type

- confluence_sources: List of confluence wikis and specific spaces and/or pages to ingest
- github_sources: List of all github organizations and the repos in them to load
- jira_sources: List of all Jira issues to ingest
- lsst_bib_sources: List of all papers and documents from the LSST bibliography files. This should contain all Rubin/LSST technotes
- docushare_sources: List of all docushare sources -- NOT YET AVAILABLE
